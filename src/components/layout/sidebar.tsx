...
export function Sidebar() {
      ...
      <Link
        ...
        className={cn(
          ...
          active
            ...
            : "text-bark/60 hover:bg-forest/5 hover:text-bark"
        )}
      >
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-64 flex-col border-r border-forest/10 bg-white">
      <div className="flex h-16 items-center gap-2 border-b border-forest/10 px-6">
        <ChefHat className="h-7 w-7 text-forest" />
        <span className="font-heading text-lg font-bold text-bark">Fridgenie Admin</span>
      </div>

      <nav className="flex-1 space-y-6 overflow-y-auto p-4">
        <div className="space-y-1">
          <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-bark/40">Core</p>
          {coreNavItems.map(renderNavItem)}
        </div>

        <div className="space-y-1">
          <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-bark/40">Analytics</p>
          {analyticsNavItems.map(renderNavItem)}
        </div>

        <div className="space-y-1">
          <p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-bark/40">Tools</p>
          {toolNavItems.map(renderNavItem)}
        </div>
      </nav>

      <div className="border-t border-forest/10 p-4">
        <div className="mb-3 truncate px-3 text-xs text-bark/40">
          {profile?.email || "Admin"}
        </div>
        <button
          onClick={signOut}
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-bark/60 transition-colors hover:bg-forest/5 hover:text-bark"
        >
